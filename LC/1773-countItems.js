...
[typei, colori, namei] describes the type, color, 
and name of the ith item. You are also given a rule 
...
Output: 1
Explanation: There is only one item matching the 
given rule, which is ["computer","silver","lenovo"]
...
["phone","gold","iphone"]. Note that the item 
...
var countMatches = function (items, ruleKey, ruleValue) {
  ...
  let currKey = keys[ruleKey];
  let matches = 0;

  for (let i = 0; i < items.length; i++) {
    if (items[i][currKey] === ruleValue) matches++;
  }
