...
of its root is 1, the level of its children 
is 2, and so on.
...
var maxLevelSum = function (root) {
  ...
  while (queue.length) {
    ...
    if (queue.length === 0) {
      ...
      // don't need to have object with arrays 
      // only update maxSum when it's > than maxSum 
      // will always get smallest level this way
      if (currSum > maxSum) {
        maxSum = currSum;
        maxLevel = currLevel;
      }
