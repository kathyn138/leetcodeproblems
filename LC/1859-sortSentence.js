...
Each word consists of lowercase and uppercase English 
letters.
...
Explanation: Sort the words in s to their original 
positions "This1 is2 a3 sentence4", then remove the numbers.
...
var sortSentence = function (s) {
  ...
  for (let i = 0; i < arr.length; i++) {
    let currWord = arr[i];
    let num = currWord[currWord.length - 1];
    let wordWithoutNum = currWord.slice(0, currWord.length - 1);
    tracker[num] = wordWithoutNum;
  }

  let resArr = [];

  for (let i = 1; i <= arr.length; i++) {
    resArr.push(tracker[i]);
  }
