...
treated as a single slash '/'. For this problem, 
any other format of periods such as '...' are 
...
Input: path = "/home/"
Output: "/home"
Explanation: Note that there is no trailing 
...
directory is a no-op, as the root level is the 
...
var simplifyPath = function (path) {
  ...
  for (let i = 0; i < separatedPath.length; i++) {
    if (separatedPath[i] == '.' || separatedPath[i] === '') continue;
    // can't go up from root directory according to example 2
    // so want to remove the folder we were in before going up
    if (separatedPath[i] === '..') {
      res.pop();
    } else {
      res.push(separatedPath[i]);
    }
  }
