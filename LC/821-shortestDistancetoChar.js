...
var shortestToChar = function (s, c) {
  ...
  // create array with indexes of c 
  for (let i = 0; i < s.length; i++) {
    if (s[i] === c) letterTracker.push(i);
  }

  // go through s
  // each time encounter a letter that isn't c, 
  // iterate through letterTracker to find min distance
  // between current letter and occurence of c
  for (let i = 0; i < s.length; i++) {
    if (s[i] === c) {
      output.push(0);
    } else {
      let min = Math.abs(i - letterTracker[0]);

      for (let j = 0; j < letterTracker.length; j++) {
        let temp = Math.abs(i - letterTracker[j]);
