...
A string is consistent if all characters in the string 
...
Example 2:
Input: allowed = "abc", words = ["a","b","c","ab",
"ac","bc","abc"]
Output: 7
Explanation: All strings are consistent.

Example 3:
Input: allowed = "cad", words = ["cc","acd","b","ba",
"bac","bad","ac","d"]
Output: 4
Explanation: Strings "cc", "acd", "ac", and "d" are 
consistent.
...
* words[i] and allowed contain only lowercase English 
letters
*/

var countConsistentStrings = function (allowed, words) {
  let tracker = new Set(allowed);
  let count = 0;

  for (let i = 0; i < words.length; i++) {
    let valid = true;
    for (let j = 0; j < words[i].length; j++) {
      if (!tracker.has(words[i][j])) {
        valid = false;
      };
