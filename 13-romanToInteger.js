...
var romanToInt = function (s) {
  ...
  for (let i = 0; i < s.length; i++) {
    if (romanLetters[s[i + 1]] > romanLetters[s[i]]) {
      val += romanLetters[s[i + 1]] - romanLetters[s[i]];

      // increment i here to prevent running last index
      // eg IV, dont want to run V by itself for this
      // as i+1 (spot after V) is undefined
